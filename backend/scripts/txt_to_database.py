...
# Initialize an empty array that will store the monthsm
    ...
    while cur_string != "ENDOFFILE":
        ...
        # print(months[0].month_id)
            ...
            if day_phase == 0:
                ...
                x1, y1, z1 = [float(value)
                              for value in stimuli[0].split(sep="/")]
                ...
                if len(stimuli) > 1:
                    x2, y2, z2 = [float(value)
                                  for value in stimuli[1].split(sep="/")]
    ...
    for week in weeks:
        ...
        end_index = min((week_ID + 1) * 7, len(days))
        ...
        weekly_stimuli = round(
            (weekly_stimuli / len(day_children) if day_children else 0), 1)
        weekly_RPE = round(
            (weekly_RPE / len(day_children) if day_children else 0), 1)
        weekly_length = round(
            (weekly_length / len(day_children) if day_children else 0), 1)

        week.total_mileage = weekly_total_mileage
        week.goal_stimuli = workout_database.create_trio(
            weekly_stimuli, weekly_RPE, weekly_length)
    ...
    for month in months:
        # Ensure we don't go out of bounds
        end_index = min((month_ID + 1) * 4, len(weeks))
        ...
        monthly_stimuli = round(
            (monthly_stimuli / len(week_children) if week_children else 0), 1)
        monthly_RPE = round(
            (monthly_RPE / len(week_children) if week_children else 0), 1)
        monthly_length = round(
            (monthly_length / len(week_children) if week_children else 0), 1)

        month.total_mileage = monthly_total_mileage
        month.goal_stimuli = workout_database.create_trio(
            monthly_stimuli, monthly_RPE, monthly_length)
...
# print(database.week.get().goal_stimuli)
