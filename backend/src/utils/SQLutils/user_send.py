...
def send_month_future(new_user, username, password):
    ...
    while new_user.month_future:
        ...
        fut = new_user.month_future.get_nowait()
...
def send_week_cycle(new_user, username, password):
    ...
    while new_user.week_history:
        ...
        query = """ INSERT INTO public.week_cycle(
            total_mileage, goal_stimuli, cycle, expected_rpe, real_rpe, complete_score, week_id, past_week, complete_mileage, month_id)
...
def send_day_cycle(new_user, username, password):
    ...
    while new_user.day_history:
        ...
        query = """ INSERT INTO public.day_cycle(
            total_mileage, goal_stimuli, lift, expected_rpe, real_rpe, complete_score, past_day, complete_mileage, week_id)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s); """   
        # fill query with appropriate user ID
    
        # 1 is a placeholder (too lazy to change shit)
        record_to_insert = (pres.total_mileage, pres.goal_stimuli, pres.lift, pres.expected_rpe, pres.real_rpe,
    ...
    while new_user.day_future:
        ...
        query = """ INSERT INTO public.day_cycle(
            ...
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s); """     
        # 1 is a placeholder (too lazy to change shit)
        record_to_insert = (fut.total_mileage, fut.goal_stimuli, fut.lift, fut.expected_rpe, fut.real_rpe,
